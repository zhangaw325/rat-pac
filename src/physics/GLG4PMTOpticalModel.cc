...
GLG4PMTOpticalModel::DoIt(const G4FastTrack& fastTrack, G4FastStep& fastStep)
{
  ...
  ipmt=GetPMTID(fastTrack);//fastTrack.GetEnvelopePhysicalVolume()->GetCopyNo();
...
# endif
    ...
    if(EfficiencyCorrection.count(ipmt)){
        if(EfficiencyCorrection[ipmt]>=0){
            ...
            G4cout<<"GLG4PMTOpticalModel["<<GetName()<<"] warning: individual efficiency correction for PMT " << ipmt << " is " << EfficiencyCorrection[ipmt] << ", resetting to 1\n";
        }
    }
/*
