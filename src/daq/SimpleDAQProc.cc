...
Processor::Result SimpleDAQProc::DSEvent(DS::Root *ds) {
  ...
  //set trigger hits flag
  DetectorConstruction* thedet = DetectorConstruction::GetDetectorConstruction();
  BWVetGenericChamber* theveto = thedet->GetVetoDet();
  std::vector<int> myflag = theveto->GetHitFlag();
  std::vector<double> mytrigE = theveto->GetHitEdep();
  std::vector<double> mytrigT = theveto->GetHodoHitTime();
  ev->ClearTriggerFlag();
  ev->ClearTriggerEdep();
  ev->ClearTriggerHitTime();
  for(int tri = 0; tri<6;tri++) {
    ev->SetTriggerFlag(tri,myflag[tri]);
    ev->SetTriggerEdep(tri,mytrigE[tri]);
    ev->SetTriggerHitTime(tri,mytrigT[tri]);
  }
  //ev->Write();
  return Processor::OK;
}
