...
public:
  EV() : TObject() {triggerflag.resize(6);triggerEdep.resize(6);triggerHitTime.resize(6);}
  virtual ~EV() {}
  ...
  //virtual InitTriggerFlag(){triggerflag.resize(6);}
  void ClearTriggerFlag(){for(int i=0;i<6;i++) triggerflag[i]=0;}
  std::vector<int> GetTriggerFlag() {return triggerflag;}
  void SetTriggerFlag(int triggerid, int flag){triggerflag[triggerid]=flag;}
  void ClearTriggerEdep(){for(int i=0;i<6;i++) triggerEdep[i]=0;}
  std::vector<double> GetTriggerEdep(){return triggerEdep;}
  void SetTriggerEdep(int triggerid, double energy){triggerEdep[triggerid]=energy;}
  void ClearTriggerHitTime(){for(int i=0;i<6;i++) triggerHitTime[i]=0;}
  std::vector<double> GetTriggerHitTime(){return triggerHitTime;}
  void SetTriggerHitTime(int triggerid, double hittime){triggerHitTime[triggerid]=hittime;}
...
protected:
  ...
  std::vector<int> triggerflag; // 1 - hit, 0 - no hit
  std::vector<double> triggerEdep;
  std::vector<double> triggerHitTime;
};
