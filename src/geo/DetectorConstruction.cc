...
G4VPhysicalVolume* DetectorConstruction::Construct() {
  ...
  try {
    ...
    info << "    glg4data + experiment = " << glg4data+experiment << newline;
  }
  ...
  try { 
    ...
    DetectorFactory::DefineWithFactory(detector_factory,ldetector);
      ...
      info << "DetectorConstruction nb1: could not access " << e.table << "[" << e.index << "]." << e.field << endl;
    try {
      ...
      if (db->Load(geo_file) == 0) {
        ...
        info << "DetectorConstruction nb2: could not access " << e.table << "[" << e.index << "]." << e.field << endl;
...
//G4VSensitiveDetector*
  veto =  new BWVetGenericChamber("/mydet/veto/genericchamber");  
