...
BWVetGenericChamber::~BWVetGenericChamber(){ResetHitFlag();ResetHitEdep();ResetHodoHitTime();}

void BWVetGenericChamber::Initialize(G4HCofThisEvent*HCE)
{  InitializeHitFlag();
   InitializeHitEdep();
   InitializeHodoHitTime();
...
{
  //G4cout<<"BWVetGenericChamber::ProcessHits begin"<<G4endl;
...
//   if(edep==0.) return true;
  ...
  G4String name = aTrack->GetVolume()->GetName();
  TString snum = name.substr(name.size()-1,1);
  int number = snum.Atoi();
  //G4cout<<"test "<<name.size()<<"\t"<<snum.Data()<<"\t"<<number<<G4endl;
  SetHitFlag(number,1);
  SetHitEdep(number,edep);
  SetHodoHitTime(number,hitTime);
