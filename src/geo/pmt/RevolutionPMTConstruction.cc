...
G4VSolid* RevolutionPMTConstruction::BuildSolid(const string &name1) {
    vector<double> zeros(fParams.rEdge.size(),0.0);
    G4Polycone *body = new G4Polycone(name1,0,CLHEP::twopi,fParams.rEdge.size(),&fParams.zEdge[0],&zeros[0],&fParams.rEdge[0]);
    return body;
}
