...
//If we start believing that B may actually help the PMT response, change default to false
  ...
  for (size_t ii = 0, id = pmtinfo.GetPMTCount(); ii < pmt_pos.size(); ii++, id++) {
  
    string pmtname = volume_name+ "_pmtenv_"+ ::to_string(id); //internally PMTs are represented by the nth pmt built, not pmtid
    
    G4ThreeVector pmtpos = pmt_pos[ii];
    G4ThreeVector pmtdir = pmt_dir[ii];
    
    // Store individual efficiency
    EfficiencyCorrection[id] = pmt_effi_corr[ii];
    ...
    if(BFieldOn){
      ...
      if(imin<0)
        ...
        if(HaveDynoData){
          ...
          for(int kk=0;kk<int(Dorie.size());kk++)
            if((pmtpos.unit()-Dpos[kk].unit()).mag()<MinDiff){
              MinDiff=(pmtpos.unit()-Dpos[kk].unit()).mag();
              mini=kk;
            }
          if(mini<0){
