...
CylindricalPMTConstruction::CylindricalPMTConstruction(DBLinkPtr table, G4LogicalVolume *mother) : PMTConstruction("cylindrical") {
    ...
    Log::Assert(fParams.photocathodeRadius>0, "CylindricalPMTConstruction: " + pmt_model + " photocathode radius must be postive");
    Log::Assert(fParams.caseThickness>0, "CylindricalPMTConstruction: " + pmt_model + " case thickness must be postive");
    Log::Assert(fParams.glassThickness>0, "CylindricalPMTConstruction: " + pmt_model + " glass thickness must be postive");
    Log::Assert(fParams.pmtRadius > fParams.caseThickness + fParams.photocathodeRadius, "CylindricalPMTConstruction: " + pmt_model + " radius is too small");
    ...
    Log::Assert(fParams.photocathode, "CylindricalPMTConstruction: " + pmt_model + " has an invalid photocathode material");
  ...
  { 
    ...
    // Glass body
    G4Tubs* glass_solid = new G4Tubs( prefix + "_glass_solid", 0., fParams.pmtRadius - fParams.caseThickness, fParams.pmtHeight - fParams.caseThickness, 0., CLHEP::twopi);
    
    // Construct inners
    const double vacuumHeight = fParams.pmtHeight - fParams.caseThickness - fParams.glassThickness;
    G4Tubs* vacuum_solid = new G4Tubs( prefix + "_vacuum_solid", 0., fParams.photocathodeRadius, vacuumHeight, 0., CLHEP::twopi);

    //Aiwu, I want to test: add a black layer in the center of PMT
    //G4Tubs* stopper_solid = new G4Tubs(prefix+"_stopper_solid",0.,fParams.photocathodeRadius,0.5,0.,CLHEP::twopi);
    //end Aiwu edit
    ...
    //Aiwu, edit
    //G4LogicalVolume* stopper_log = new G4LogicalVolume(stopper_solid,fParams.outerCase,prefix+"_stopper_logic");
    //end Aiwu edit
    ...
    //Aiwu edit
/*
    stopper_phys = new G4PVPlacement
    (0,
     noTranslation,
     stopper_log,
     prefix+"_stopper_phys",
     vacuum_log,
     false,
     0);
*/
  ...
  G4VSolid* CylindricalPMTConstruction::BuildSolid(const string &name1) {
    G4Tubs* body = new G4Tubs(name1, 0., fParams.pmtRadius, fParams.pmtHeight, 0., CLHEP::twopi);
    return body;
  }

  G4PVPlacement* CylindricalPMTConstruction::PlacePMT(
    ...
    G4PVPlacement *body_phys = new G4PVPlacement(pmtrot, pmtpos, name1, logi_pmt, mother_phys,  booleanSolid, copyNo);

    // photocathode surface
    new G4LogicalBorderSurface(name1+"_photocathode_logsurf1", vacuum_phys, glass_phys, fParams.photocathode);
