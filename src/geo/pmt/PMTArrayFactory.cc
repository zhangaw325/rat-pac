...
namespace RAT {
    ...
    G4VPhysicalVolume *PMTArrayFactory::Construct(DBLinkPtr table){
        ...
        PMTInfoParser pmtinfo1(lpos_table,mother_name);
        
        const vector<G4ThreeVector> &pmtinfo_pos = pmtinfo1.GetPMTLocations();
        const vector<G4ThreeVector> &pmtinfo_dir = pmtinfo1.GetPMTDirections();
        const G4ThreeVector local_offset = pmtinfo1.GetLocalOffset();
        ...
        */
        ...
        return ConstructPMTs(table,pos,dir,pmtinfo1.GetTypes(),pmtinfo1.GetEfficiencyCorrections());
    }
