...
void Gsim::BeginOfRunAction(const G4Run* /*aRun*/) {
  ...
  std::cout<<"The number of PMTs in is "<<numModels<<std::endl;
  ...
  for (size_t i = 0; i < numModels; i++) {
    ...
    try {
      std::cout<<"Trying get PMT charge"<<std::endl;
      fPMTCharge[i] = new RAT::PDFPMTCharge(modelName);
      std::cout<<"PDFPMTCharge "<<fPMTCharge[i]<<std::endl;
    } catch (DBNotFoundError& e) {
      //fallback to MiniCleanPMTCharge if nothing else avaliable
      std::cout<<"Prepare for using MinicleanPMTCharge "<<std::endl;
      fPMTCharge[i] = new RAT::MiniCleanPMTCharge();
      std::cout<<"MiniPMTCharge "<<fPMTCharge[i]<<std::endl;
    }
  }

  std::cout<<"Gsim::Before run starts"<<std::endl;
...
void Gsim::BeginOfEventAction(const G4Event* anEvent) {
  ...
  std::cout<<"Gsim::BeginOfEventAction"<<std::endl;
  ...
  std::cout<<"Gsim::EndOfBeginOfEventAction"<<std::endl;
}

void Gsim::EndOfEventAction(const G4Event* anEvent) {
    std::cout<<"Gsim::Begin EndOfEventAction"<<std::endl;
