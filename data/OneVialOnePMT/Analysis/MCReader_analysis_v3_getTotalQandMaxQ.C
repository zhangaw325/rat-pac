...
int main(int argc, char **argv) {
    ...
    if (argc != 2 ) {
        cout << "Usage: ./MCReader_analysis_v3_EvtByEvt output.root timename " << endl;
    }

    string dir = "/media/sf_E_DRIVE/Research/BNL-EDG/Simulation/MC_data_comparison_20170915/MCData_HodoTrigOnly_50kEvts_onetonWater/addOpticalCookies_yellowish/";     //directory that contains the ROOT Tree files
    const int numberofinputfiles = 5; 
    //argv[3] is the output root file. Note ONLY ONE root file is output
    string head = "/cosmic_run1025_HodoOnly_run";
    string tail = "_10kEvts.root";
    string inputrootfile[numberofinputfiles];
    fstream fin_energy(argv[2],ios::in);
    char buffer[2];
    ...
    for(int i=0;i<numberofinputfiles;i++){
     // if(i!=0 && i!=2) continue;
      ...
      //itoa(i+1,buffer,10);
      inputrootfile[i]=dir+head+ss.str()+tail;
    }
...
//    TH2F* hPMTTimeVsNHit = new TH2F("PMTTimeVsNHit","",100000,0,10000,401,-0.5,400.5);
    ...
    for(int i=0; i<pmtnb; i++){
        for(int j=0;j<3;j++){
            ...
            //cout<<hName<<endl;
            hCharge[i][j] = new TH1F(hName,"",5000,0,500);
...
for(int ffff=0; ffff<numberofinputfiles; ffff++){ // start loop reading the root tree files
   ...
   */
        ...
        //gTest1->GetXaxis()->SetTitle("ID"); gTest1->GetYaxis()->SetTitle("Track ID"); gTest1->SetMarkerStyle(20);gTest1->SetMarkerSize(0.6);
        //cTest1->cd();  gTest1->Draw("AP"); cTest1->SaveAs("TrackID_vs_ID_evt1.root"); cTest1->Close();
/*
        if(i>=0 && i<=15){
          gTest->SetMarkerStyle(20); gTest->SetMarkerSize(0.6);
          c = new TCanvas();
          gTest->GetXaxis()->SetTitle("X");
          gTest->GetYaxis()->SetTitle("Y"); 
          gTest->GetZaxis()->SetTitle("Z");  
          gTest->Draw("P");  
          gPad->Update();
          char thistrackname[25];sprintf(thistrackname,"TestTrack_Evt%d.png",i);
          c->SaveAs(thistrackname);
          char thistrackname1[25];sprintf(thistrackname1,"TestTrack_Evt%d.root",i);
          c->SaveAs(thistrackname1);
          c=NULL;
          for(int mm=0;mm<graphcnt++;mm++) gTest->RemovePoint(mm);
          graphcnt=0;
        }
*/
...
//        energy = 10.0;
        ...
        //fout<<"Event "<<i<<" ==== nPrim "<<nPrim<<" NumPE "<<npe
        //    <<" MCTrackCount "<<mctrackcnt
            //<<" energyCentroid ("<<energyCentroid[0]<<", "<<energyCentroid[1]<<", "<<energyCentroid[2]<<")"
            //<<" totalScintEdep "<<totalScintEdep
            //<<" pdg code "<<pdgcode<<" ke "<<ke<<" momentum ("<<momentum[0]<<", "<<momentum[1]<<", "<<momentum[2]<<")"
            //<<" ev_pos ("<<evpos[0]<<", "<<evpos[1]<<", "<<evpos[2]<<")"
        //    <<" nHits "<<nHits
            //<<" charge dif "<<totalq-tempq<< " uDCnt "<< ds->GetUserDataCount() 
        //   <<endl; 

//here I look for proper trigger types
        //trigger from plastic scintillator: (H0 && H2) || (H1 && H3)
        if( (myTriggerEdep[0]>0.0 || myTriggerEdep[2]>0.0) && (myTriggerEdep[1]>0.0 || myTriggerEdep[3]>0.0) ) {ispstrigger = true; hodotrigcnts++; flag=true;}
        // multiplicity trigger
        if(nHits>6){
            ismultitrigger = true;
        }
        //fout<<"\t\t\t"<<ispstrigger<<"\t"<<ismultitrigger<<endl;
//end finding trigger types

        for(int tri=0;tri<6;tri++){
            //cout<<myTriggerFlag[tri]<<"\t"<<myTriggerEdep[tri]<<"\t"<<endl;
           if(myTriggerEdep[tri]>0){
             if(ispstrigger==true/*&& ismultitrigger==false*/) {hEdepInPS[tri][0]->Fill(myTriggerEdep[tri]); hHitTimeInPS[tri][0]->Fill(myTriggerHitTime[tri]);}
             if(ismultitrigger==true /*&& ispstrigger==false*/) {hEdepInPS[tri][1]->Fill(myTriggerEdep[tri]); hHitTimeInPS[tri][1]->Fill(myTriggerHitTime[tri]);}
             hEdepInPS[tri][2]->Fill(myTriggerEdep[tri]);
             hHitTimeInPS[tri][2]->Fill(myTriggerHitTime[tri]);
             //std::cout<<tri<<"\t"<<myTriggerHitTime[tri]<<std::endl;
           }
        }
        ...
        for(int j=0; j<nHits; j++){
           ...
           // fout << "\t\t Hit " << j << " on PMT " << id << " at (" << hitpos[0] << ", "<< hitpos[1] << ", " << hitpos[2] << ')' << ", npe "<< pmtchargeholder[id] << endl;
            ...
            if(ispstrigger==true /*&& ismultitrigger==false*/){ // hodo triggers
                hTime[pmtidholder[j]][0]->Fill(pmttimeholder[pmtidholder[j]]);
                hCharge_vs_Time[pmtidholder[j]][0]->Fill(pmttimeholder[pmtidholder[j]],pmtchargeholder[pmtidholder[j]]);
                hCharge[pmtidholder[j]][0]->Fill(pmtchargeholder[pmtidholder[j]]);
//                hPMTChargeMap_Hodo->Fill(pmt_x[pmtidholder[j]],pmt_y[pmtidholder[j]],pmtchargeholder[pmtidholder[j]]);
                hPMTChargeMap_Hodo->Fill(hitpos[0],hitpos[1],pmtchargeholder[pmtidholder[j]]);
                //cout<<pmtidholder[j]<<"\t"<<pmtchargeholder[pmtidholder[j]]<<"\t"<<ismultitrigger<<endl;
            }
            if(ismultitrigger==true /*&& ispstrigger==false*/){
                hTime[pmtidholder[j]][1]->Fill(pmttimeholder[pmtidholder[j]]);
                hCharge_vs_Time[pmtidholder[j]][1]->Fill(pmttimeholder[pmtidholder[j]],pmtchargeholder[pmtidholder[j]]);
                hCharge[pmtidholder[j]][1]->Fill(pmtchargeholder[pmtidholder[j]]);
            }
        ...
        if(ispstrigger==true)  hTotalChargeVsNHit_Hodo->Fill(tempq,nHits);
        ismultitrigger = false; ispstrigger = false;
        //if(maxq>0){
   ...
   foutroot->cd();
    ...
    for(int i=0; i<pmtnb; i++){
      for(int j=0; j<3; j++){
        ...
        if(j==2){
            hPMTChargeMapAverage->Fill(pmt_x[i],pmt_y[i],hCharge[i][j]->GetMean()); //all triggers
            hPMTTimeMapAverage->Fill(pmt_x[i],pmt_y[i],hTime[i][j]->GetMean());
        }
        if(j==0){ hPMTChargeMap_HodoAverage->Fill(pmt_x[i],pmt_y[i],hCharge[i][j]->GetMean());//hodo triggers
            hPMTTimeMapAverage_hodo->Fill(pmt_x[i],pmt_y[i],hTime[i][j]->GetMean());
        }
        if(j==1) hPMTChargeMap_MultiAverage->Fill(pmt_x[i],pmt_y[i],hCharge[i][j]->GetMean());//multiplicity triggers
      }
    }
    hPMTChargeMap->Write();
